        ...
        <div className="max-w-[1920px] mx-auto px-6 md:px-12 flex justify-between items-center relative">
            ...
            {NAV_ITEMS.map((item) => (
              ...
              >
                <Link
                  ...
                  className="flex items-center gap-1 font-display text-xs font-bold uppercase tracking-[0.1em] text-text-main hover:text-[#719B31] transition-colors py-2"
                >
                  {item.label}
                  {item.children && (
                    <span
                      className={`text-[10px] transition-transform duration-300 ${hoveredItem === item.label ? 'rotate-180 text-[#719B31]' : ''}`}
                    >
                      53b
                    </span>
                  )}
                ...
                <span className={`absolute -bottom-1 left-0 w-full h-[2px] bg-[#719B31] transform origin-left transition-transform duration-300 ${hoveredItem === item.label ? 'scale-x-100' : 'scale-x-0'
                  }`} />
              </div>
            ))}
          </nav>
          ...
          <div className="hidden md:flex items-center gap-6">
            <a
              ...
              className="bg-[#719B31] text-white px-6 py-3 font-display font-bold text-xs uppercase tracking-[0.1em] hover:bg-[#567524] transition-colors"
            >
              24/7 Praesidium
            </a>
          </div>
                    ...
                    {NAV_ITEMS.find(i => i.label === hoveredItem)?.children?.map(child => (
                      <li key={child.label}>
                        <Link
                          ...
                          className="group flex items-center gap-3 text-text-main hover:text-[#719B31] transition-colors font-sans font-medium text-lg"
                        >
                          <span className="text-sm opacity-0 -ml-4 group-hover:opacity-100 group-hover:ml-0 transition-all duration-300 text-[#719B31]">9d</span>
                          {child.label}
                        </Link>
                      </li>
                    ))}
                ...
                <div className="col-span-5 col-start-8">
                  <div className="aspect-video bg-bg-panel relative overflow-hidden group cursor-pointer">
                    ...
                    <div className="absolute bottom-8 left-8 bg-white p-6 shadow-lg max-w-xs">
                      <p className="font-display text-xs text-[#719B31] uppercase tracking-widest mb-2">Ministerium Insigne</p>
            ...
            <nav className="flex flex-col gap-8 pb-12">
              {NAV_ITEMS.map((item) => (
                <div key={item.label} className="border-b border-gray-100 pb-4">
                  ...
                  >
                    {item.label}
                    {item.children && <span className="text-[#719B31]">9d</span>}
                  </Link>
                  {item.children && (
